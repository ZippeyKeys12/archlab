...
#    Copyright (C) Randal E. Bryant, David R. O'Hallaron, 2019     #
####################################################################
...
####################################################################
#    Description of changes to the code                            #
####################################################################
# 1. Add iaddq, exact same as hw6
#
# 2. Implement load forwarding mentioned in pipe-lf.hcl.
#    If there are two instructions where the first one
#    reads memory and the second one uses that result to
#    write, then there doesn't need to be a stall, because
#    you can forward the value read in the memory stage,
#    to the second instructions execute stage, because it
#    only needs the value in its memory stage.
#    This works for { IMRMOVQ, IPOPQ } to { IRMMOVQ, IPUSHQ }
#
# 3. Predict jump as taken when it's backward because it likely
#    a loop, predict not taken when it is forward. Mentioned in
#    pipe-btfnt.hcl.
#
# 4. If the prediction was not taken, and D_valP is M_valE, don't
#    bubble it. If M_valA == M_valE, then don't bubble either.
#    If any of the instructions are jumps, all bets are off.
#
# 5. The ret instruction stalls for 3 cycles, however, if the
#    previous instruction is a pushq, the register used contains
#    the return address. Therefore, we can forward it instead of
#    stalling for 3 cycles. Would've been very useful if I had
#    gotten jump tables to work.
...
word f_pc = [
	...
	M_icode == IJXX && M_ifun != 0 &&
	 M_valE > M_valA && M_Cnd &&
	 # Decrease MP
	 (D_valP < M_valE || D_icode == IJXX || E_icode == IJXX) : M_valE;
	# Forward IPUSHQ to IRET
	E_icode == IPUSHQ && D_icode == IRET : M_valA;
	# Completion of IRET instruction if bubbled
	W_icode == IRET : W_valM;
...
word aluA = [
	...
	E_icode in { IJXX } : E_valC; # pass valC to M_valE
...
bool F_stall =
	...
	# Stalling at fetch while IRET passes through pipeline
	# Unless IPUSHQ forwards the next instruction
...
bool D_bubble =
	...
	(E_icode == IJXX && E_ifun != 0 &&
	 E_valC > E_valA && e_Cnd &&
	 # Decrease MP
	 (F_predPC < E_valC || f_icode == IJXX || D_icode == IJXX)) ||
